...
async function writeMarkdownReport(filePath: string, report: ImportReport): Promise<void> {
  const lines = [
    ...
    `- Renamed files: ${report.renamedFiles?.length ?? 0}`,
    `- Deleted files (recycle bin): ${report.deletedFiles?.length ?? 0}`,
    `- Icon updates: ${report.iconUpdates?.length ?? 0}`,
    ...
    '## Renamed Files',
    '',
    ...(report.renamedFiles?.length ? report.renamedFiles.map((rename) => `- ${rename.from} -> ${rename.to}`) : ['- None']),
    '',
    '## Icon Updates',
    '',
    ...(report.iconUpdates?.length ? report.iconUpdates.map((icon) => `- ${icon.category}: IconID ${icon.iconId}`) : ['- None']),
    '',
